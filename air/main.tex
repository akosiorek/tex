...
\section{Inference through an Approximate Posterior}
    ...
    \end{equation}
    Since the hidden state $\bm{h}^i$ does not depend on any of the latent variables, there is no need to integrate over the hidden state \todo{check with the STORN paper}. All of the distribution parameters $\omega$ are computed before any of the distributions is used or even instantiated. That hints at what the implementation might look like: a recurrent neural network should compute the parameters of all the probability distributions, but all sampling can be done outside in a feed-forward fashion.
